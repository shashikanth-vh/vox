...
from app.seed.fi_master import seed_fi_master
...
async def run(tenant_code: str, tenant_name: str) -> None:
    ...
    async with sm() as session:
        ...
        fi_added = await seed_fi_master(session, tenant_id)
        await session.commit()
    log.info("bootstrap complete: tenant=%s (%s), ref_values~%d, checklist=+%d, template=+%d, "
             "fi_master=+%d",
             tenant_code, tenant_id, ref_added, checklist_added, template_added, fi_added)
    print(f"Bootstrap complete. Tenant '{tenant_code}' ready ({tenant_id}). "
          ...
          f"FI master lenders added: {fi_added}. "
