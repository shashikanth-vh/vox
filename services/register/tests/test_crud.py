...
async def test_converted_deal_remarks_keep_their_lineage_line(client: AsyncClient):
    """The conversion record is a LOCKED first line of a converted deal's remarks:
    the desk revises its own remark below it freely, a save that drops or edits the
    line is refused, and Admin (the correction lane) may rewrite the whole field."""
    ADMIN = {"X-User-Email": "admin@evamfinance.com", "X-User-Roles": "Admin"}
    DESK = {"X-User-Email": "mgmt@evamfinance.com", "X-User-Roles": "Management"}
    eid = (await client.post("/v1/entities",
                             json={"code": "LINE", "legal_name": "LINE"})).json()["id"]
    lineage = "Converted from lead LD-042 (Line Co). (approved by auto-approval@policy)"
    deal = (await client.post("/v1/deals", json={
        "entity_id": eid, "code": "LINE", "remarks": lineage})).json()

    # Desk revises its remark under the lineage — again and again — freely.
    for note in ("Godrej terms received", "IM circulated to 4 banks"):
        r = await client.patch(f"/v1/deals/{deal['id']}",
                               json={"remarks": f"{lineage}\n{note}"}, headers=DESK)
        assert r.status_code == 200, r.text
    assert (await client.get(f"/v1/deals/{deal['id']}")).json()["remarks"] \
        == f"{lineage}\nIM circulated to 4 banks"

    # Dropping or editing the lineage line is refused, and the refusal teaches.
    r = await client.patch(f"/v1/deals/{deal['id']}",
                           json={"remarks": "My own story now"}, headers=DESK)
    assert r.status_code == 422 and "conversion record" in r.text

    # Admin is the correction lane: a full rewrite is allowed (and audited).
    r = await client.patch(f"/v1/deals/{deal['id']}",
                           json={"remarks": "Record corrected by Admin"}, headers=ADMIN)
    assert r.status_code == 200, r.text

    # A deal with NO lineage keeps a free-text remarks field.
    plain = (await client.post("/v1/deals", json={"entity_id": eid, "code": "LINE"})).json()
    r = await client.patch(f"/v1/deals/{plain['id']}",
                           json={"remarks": "anything at all"}, headers=DESK)
    assert r.status_code == 200, r.text
