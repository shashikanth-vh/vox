"""PULSE e2e fixtures — the Register runs as a real uvicorn server; PULSE in-process."""
...
os.environ["PULSE_REGISTER_BASE_URL"] = f"http://127.0.0.1:{REGISTER_PORT}"
os.environ["PULSE_REGISTER_API_KEY"] = "test-key"
os.environ["PULSE_REGISTER_TENANT"] = "EVAM"
os.environ["PULSE_LOG_LEVEL"] = "WARNING"
os.environ["PULSE_SOURCES"] = '[{"name": "sample", "kind": "sample"}]'
...
async def pulse() -> AsyncIterator[httpx.AsyncClient]:
    ...
    async with app.router.lifespan_context(app), httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://pulse",
    ) as client:
        yield client
