"""VocX e2e fixtures — the Register runs as a real uvicorn server; VocX in-process."""
...
os.environ["VOCX_REGISTER_BASE_URL"] = f"http://127.0.0.1:{REGISTER_PORT}"
os.environ["VOCX_REGISTER_API_KEY"] = "test-key"
os.environ["VOCX_REGISTER_TENANT"] = "EVAM"
os.environ["VOCX_LOG_LEVEL"] = "WARNING"
...
async def vocx() -> AsyncIterator[httpx.AsyncClient]:
    ...
    async with app.router.lifespan_context(app), httpx.AsyncClient(
        transport=ASGITransport(app=app), base_url="http://vocx",
    ) as client:
        yield client
