...
class Transcriber:
    """Interface: transcribe(audio) -> transcript dict."""

    def transcribe(self, audio: AudioInput, language: str | None = None) -> dict[str, Any]:
...
class FasterWhisperTranscriber(Transcriber):
    ...
    def transcribe(self, audio: AudioInput, language: str | None = None) -> dict[str, Any]:
        with self._lock:
            return self._transcribe_locked(audio, language)

    def _transcribe_locked(self, audio: AudioInput, language: str | None = None) -> dict[str, Any]:
        import io
        model = self._load()
...
# --- remote API (swap-in later) ----------------------------------------------
class APITranscriber(Transcriber):
    """A remote Whisper-compatible endpoint (OpenAI-style multipart). Endpoint and
    key come from env so no secrets live in config."""

    def __init__(self, endpoint_env: str = "VOX_STT_API_URL", key_env: str = "VOX_STT_API_KEY",
                 model: str = "whisper-1", timeout: int = 120):
...
def build_transcriber(config: dict[str, Any]) -> Transcriber:
    ...
    if backend == "api":
        api = stt.get("api", {})
        return APITranscriber(api.get("endpoint_env", "VOX_STT_API_URL"),
                              api.get("key_env", "VOX_STT_API_KEY"),
