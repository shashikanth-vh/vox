...
def build_known_names_block(company_names: Iterable[str] | None = None) -> str:
    ...
    lines.append(
        ...
        "- location is the MEETING VENUE only — where this conversation happened "
        "('met them at their Whitefield office' -> 'Whitefield'). Project sites, "
        "plant locations and state names ('10,000 MW plant in Mundra', 'projects "
        "in Tamil Nadu and Bihar') NEVER fill location — each business block has "
        "its own field for them: lending.project_location and "
        "syndication.project_location for where the financed/syndicated "
        "projects sit, asset_monetisation.asset_location for the asset on "
        "offer. Different lanes may carry DIFFERENT places from one meeting "
        "('Tamil Nadu loan, Chikkaballapur asset'). If the venue is not "
        "spoken, location is null.\n"
        "- sector/subsector describe the COUNTERPARTY'S business, chosen from "
        "the given taxonomy: a company building or operating solar plants is "
        "Renewables / Solar-Developer; an EPC contractor Solar-EPC; a module "
        "manufacturer Solar-OEM. Then fill that subsector's key-data "
        "(subsector_details) fields from figures actually spoken — capacity in "
        "MW, portfolio/stage, offtake or PPA status — and leave the rest null "
        "rather than guessing.\n"
        "- One meeting can carry SEVERAL asks: a loan request and a debt "
